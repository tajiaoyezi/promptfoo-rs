...
> git 协作宽松：直接在当前 trunk（初始化时为 master）上工作，不需要 worktree / feature branch / PR 流程。
> **但 S2V 开发规范的核心方法论仍 100% 必守**——见下方"必守清单"。
>
> **任何 agent（含外部 Codex / Cursor / Aider）进入本仓库时第一件事：读完本文件 + `docs/s2v-adapter.md`**。
...
4. **§2.5 三段 commit 节律**：每个 task 至少 RED commit + GREEN commit（REFACTOR 可选）
   - solo 档下三段 commit 都直接落当前 trunk，仍是 TDD 履迹的可验证性，不可省略
...
#    不靠退出码）。src/ crates/ tests/ viewer/ npm/ compatibility/：读 adapter §Source And Test Areas > Unit test areas bullet list，空格分隔多 pathspec（无外层引号）。
s2v_baseline_green "src/ crates/ tests/ viewer/ npm/ compatibility/" || exit 1
...
solo 档虽然直接在当前 trunk（初始化时为 master）上工作，但 agent 误开 chore/feat 分支 / 误 cherry-pick / 误 rebase 仍会发生。每次 commit 必校验：

```bash
EXPECTED=$(git branch --show-current)   # solo 档通常是当前 trunk
