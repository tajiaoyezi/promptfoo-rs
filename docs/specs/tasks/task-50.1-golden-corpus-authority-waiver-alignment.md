...
- [ ] **AC1** (ADR-012): frozen baseline golden corpus `active_blocker_count=0`, `waived_blocker_count=24`, audit `blocker_count=24`.
- [ ] **AC2** (v1 policy): waived rows reference manifest `item_id`; unresolved local P0 blockers still count as active.
- [ ] **AC3** (ADR-009): quality gate clears golden-corpus blocker when `active_blocker_count=0`; audit count remains in golden artifact.
- [ ] **AC4** (release.md): runtime smoke regenerates gates; `perfect_refactor_claim_allowed` follows all gates (golden active zero + authority/publication readiness).

## 7. SDD / BDD / TDD Traceability

| Acceptance Criterion | BDD Scenario | TDD Test | Integration / E2E Test | Verification | Status |
|---|---|---|---|---|---|
| AC1 | SCEN-50.1.1 | TEST-50.1.1 | tests/product_baseline_golden_burndown.rs | install, typecheck, unit-test, runtime-smoke, build | Not Started |
| AC2 | SCEN-50.1.1 | TEST-50.1.2 | tests/current_latest_golden_corpus.rs | install, typecheck, unit-test, build | Not Started |
| AC3 | SCEN-50.1.1 | TEST-50.1.3 | tests/product_baseline_golden_burndown.rs | install, typecheck, unit-test, integration, build | Not Started |
| AC4 | SCEN-50.1.1 | TEST-50.1.4 | tests/product_baseline_golden_burndown.rs | install, typecheck, unit-test, e2e, runtime-smoke, build | Not Started |
