...
|---|---|---|---|---|
| 45.1 | rust-promptfoo-binary-alias | ../tasks/task-45.1-rust-promptfoo-binary-alias.md | Ready | 让 Cargo release 同时生成 `promptfoo` 和 `promptfoo-rs` 两个等价 Rust CLI 入口 |
| 45.2 | npm-promptfoo-bin-shims | ../tasks/task-45.2-npm-promptfoo-bin-shims.md | Ready | 让 npm wrapper 暴露 `promptfoo`、`promptfoo-rs`、`pf` bin shim 并保持本地 smoke 可验证 |
