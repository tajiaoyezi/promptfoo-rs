...
|---|---|---|---|---|
| 6.1 | upstream-harness-runner | ../tasks/task-6.1-upstream-harness-runner.md | Draft | 执行 upstream promptfoo@0.121.13 与 promptfoo-rs 的同输入 fixture runner |
| 6.2 | golden-diff-release-gate | ../tasks/task-6.2-golden-diff-release-gate.md | Draft | 实现 golden diff 分类、coverage report 和 stable release gate |
...
- [ ] Phase 6 所有 task spec 顶部 Status 已从 Draft 经用户审核后推进到 Ready，再由实施 agent 完成到 Done。
