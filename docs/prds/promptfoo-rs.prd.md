...
| 41 | current-latest-github-head-drift-refresh | 刷新 GitHub default branch HEAD 漂移到 `9d7d810c2118c63cb537bf05ea2d34c12bd22066`，同时保留 npm latest/release `0.121.14` 并重跑 downstream gates | `compatibility/inventory/current-latest-target.json` + `docs/compatibility/current-latest.lock.md` + `tests/current_latest_github_head_drift_refresh.rs` + `docs/compatibility/matrix.md` + `test/features/perfect-refactor-parity.feature` | 40 | 否 |
| 42 | current-latest-2ca16c-head-refresh | 刷新 GitHub default branch HEAD 漂移到 `2ca16c59b64e0afca10533de0f817c0d24eba20a`，同时保留 npm latest/release `0.121.14` 并重跑 downstream gates | `compatibility/inventory/current-latest-target.json` + `docs/compatibility/current-latest.lock.md` + `tests/current_latest_github_head_drift_refresh.rs` + `docs/compatibility/matrix.md` + `test/features/perfect-refactor-parity.feature` | 41 | 否 |
| 48 | current-latest-0.121.15-target-refresh | 刷新 current-latest target lock 到 2026-06-07 观测到的 promptfoo 0.121.15 与 GitHub HEAD `c54a30668ad8319d76c20ae96e6680ad6c51a2c6`，并重跑 downstream gates | `compatibility/inventory/current-latest-target.json` + `docs/compatibility/current-latest.lock.md` + `tests/current_latest_0_121_15_target_refresh.rs` + `docs/compatibility/matrix.md` + `test/features/perfect-refactor-parity.feature` | 47 | 否 |
