...
Task 19.4 implementation adds the combined external authority gate at `target/release-gates/external-authority-blockers.json`. It keeps provider product/account/credential blockers and publication authority blockers itemized with required decisions, safe local fallbacks, and release impact. The gate preserves blocked or waived-with-boundary status only; it does not claim external product parity or public publication while credentials, accounts, legal/brand approval, or authority evidence are absent.

Task 20.1 implementation adds cross-ledger provider source accounting reconciliation. Current artifacts report 22 fixture-covered provider rows excluded from source `remaining_p0_blockers`, 15 provider external-authority blockers retained, 7 config external blockers retained, `provider_source_generic_blocker_count=0`, and `p0_accounting_blocker_count=22`. This fixes the Phase 19 double-counting gap between source accounting and provider burndown while preserving the boundary that external authority blockers still prevent a perfect refactor claim.

Task 20.2 implementation adds `target/release-gates/perfect-refactor-claim.json` and records `perfect_refactor_claim_allowed=false` even when local stable release gates can pass. The claim contract keeps source accounting, current-upstream, external authority, and publication authority blockers itemized by source artifact, so local stable readiness remains distinct from promptfoo perfect-refactor completion.

Task 21.1 implementation adds `target/release-gates/upstream-distribution-target.json` and separates npm `promptfoo` core package metadata from GitHub repository HEAD and observed GitHub release tags. This lets the audit preserve frozen-baseline compatibility for the published npm core package when the npm metadata still matches `0.121.13`, while continuing to block repository-current perfect-refactor claims when GitHub HEAD or a non-core release tag has drifted.

Task 22.1 implementation adds `target/release-gates/perfect-refactor-unblock-packet.json` as a blocker handoff artifact. The packet keeps `perfect_refactor_claim_allowed=false`, groups source-only config blockers separately from provider external-authority blockers, preserves publication and current-upstream rebaseline requirements, and marks every decision item `auto_resolvable=false`. It is evidence for what external decisions remain, not evidence that those decisions have been granted.

Task 23.1 implementation updates the upstream distribution target gate so the GitHub dynamic latest release ref comes from latest release metadata and is recorded in `upstream-distribution-target.json.github.source`. This fixes stale release-observation evidence, but it still does not prove current-upstream same-ref parity or public perfect-refactor completion; external authority, publication, source accounting, and current rebaseline blockers remain in force.
