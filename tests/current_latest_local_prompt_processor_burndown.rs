...
fn fixture_dir(name: &str) -> std::path::PathBuf {
    let dir = std::env::temp_dir().join(format!(
        "promptfoo-rs-current-latest-local-prompt-processor-{name}-{}",
        std::process::id()
    ));
...
fn write_prompt_processor_source(root: &Path) {
    for relative in local_prompt_processor_sources()
        .iter()
        .chain(script_prompt_processor_sources().iter())
    {
        write_file(root, relative, "export const promptProcessor = true;");
    }
}

fn local_prompt_processor_sources() -> &'static [&'static str] {
    &[
...
#[test]
fn test_32_1_1_local_prompt_processor_rows_have_native_fixture_evidence() {
    /* TEST-32.1.1 */
    ...
    for source in local_prompt_processor_sources() {
...
#[test]
fn test_32_1_3_script_and_rust_extractors_emit_equivalent_local_processor_evidence() {
    /* TEST-32.1.3 */
...
#[test]
fn test_32_1_4_golden_and_quality_keep_script_processor_blockers_visible() {
    /* TEST-32.1.4 */
