...
fn fixture_dir(name: &str) -> std::path::PathBuf {
    let dir = std::env::temp_dir().join(format!(
        "promptfoo-rs-current-latest-evaluator-runtime-fixture-{name}-{}",
        std::process::id()
    ));
...
#[test]
fn test_40_1_1_rust_promotes_evaluator_runtime_to_native_fixture() {
    /* TEST-40.1.1 */
    ...
    let row = evaluator_runtime_row(&inventory.rows);
    ...
    assert!(
        row.blocker_reason
            .as_deref()
            .unwrap_or_default()
            .contains("covered by deterministic evaluator runtime fixture evidence"),
        "{row:#?}"
    );
...
#[test]
fn test_40_1_2_shell_promotes_evaluator_runtime_to_native_fixture() {
    /* TEST-40.1.2 */
...
#[test]
fn test_40_1_3_golden_has_no_evaluator_runtime_release_blocker() {
    /* TEST-40.1.3 */
    ...
    assert!(
        ...
        "evaluator runtime should not remain a release blocker: {blockers:#?}"
    );
...
#[test]
fn test_40_1_4_evaluator_runtime_fixture_reference_is_stable() {
    /* TEST-40.1.4 */
...
fn evaluator_runtime_row(
    ...
    rows.iter()
        ...
        .unwrap_or_else(|| panic!("missing evaluator runtime row: {rows:#?}"))
}
